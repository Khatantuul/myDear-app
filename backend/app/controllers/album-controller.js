...
const defaultAlbumIDs = [
  "674f3774dd66afeb37d40872",
  "674f3256dd66afeb37d407f0",
  "674f2d3edd66afeb37d40597",
  "674e887e466597368d3f14b4",
];
...
export const createAlbum = async (req, res) => {
  ...
  uploadMiddleware(req, res, async (err) => {
    ...
    try {
      ...
      const album = await albumServices.saveAlbum({
        title,
        description,
        tags,
        creator,
      ...
      const uploadPromises = photos.map(async (photoFile, idx) => {
        ...
        if (photoInfo[idx].note || photoInfo[idx].tags) {
          let photo = await photoServices.savePhoto({
            note: photoInfo[idx].note,
            tags: photoInfo[idx].tags,
          });
          const compressed = {
            ...photoFile,
  ...
  try {
    ...
    if (albums.length === 0) {
      ...
      for (let album of albums) {
        const imageKeys = await getAlbumImageKeys(
          "652ecc958a61b83ac4f4afcf",
          album._id,
          preview
        );
...
export const fetchAllAlbumPhotos = async (req, res) => {
  ...
  try {
    ...
    if (defaultAlbumIDs.includes(albumId)) {
      imageKeys = await getAlbumImageKeys(
        "652ecc958a61b83ac4f4afcf",
        albumId,
        false
      );
...
export const deletePhotosFromAlbum = async (req, res) => {
  try {
    ...
    const updatedAlbum = await albumServices.updateAlbumRemovePhotos(
      albumId,
      photos
    );
